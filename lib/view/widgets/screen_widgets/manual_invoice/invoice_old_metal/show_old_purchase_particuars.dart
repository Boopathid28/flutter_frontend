  ...
  Widget build(BuildContext context) {
      ...
      content: SingleChildScrollView(
        child: Container(
          ...
          child: Obx(() => ListView.separated(
            shrinkWrap: true,
            itemCount: _invoiceController.oldPurchaseParticulars.length,
              ...
              return Card(
                ...
                child: Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      CustomLabelFilter(label: _invoiceController.oldPurchaseParticulars.value[index].oldGoldBillNo??""),
                      BillingLabel(label: "₹ ${_invoiceController.oldPurchaseParticulars.value[index].totalAmount??"0.00"}")
                    ],
                  ),
                ),
              );
    ...
    actions: [
      PrimaryTextButton(text: 'Clear All', onPressed: () {
        _invoiceController.oldPurchaseParticulars([]);
        _invoiceController.calculateOldMetals();
        Get.back();
      })
    ],
    );
