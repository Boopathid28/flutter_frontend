...
class StockAssignService {
  static Future fetchStockAssignListList({
    FetchStockAssignListPayload? payload,
  }) async {
    var response = await ApiCalls.postWithToken(
        url: stockassignListEndPoint, body: payload!.toJson());
    if (response != null) {
      if (response['status'] == 200) {
        List<StockAssignListData> tempList = <StockAssignListData>[];
        for (var item in response['data']['list']) {
          tempList.add(StockAssignListData.fromJson(item));
        }
        return {
          "data": tempList,
  ...
  static Future createStockList(
      {CreateStockAssignPayload? payload,
      required BuildContext context}) async {
    var response = await ApiCalls.postWithToken(
        url: stockassignEndPoint, body: payload!.toJson());
    if (response != null) {
      if (response['status'] == 200) {
        toastification.show(
