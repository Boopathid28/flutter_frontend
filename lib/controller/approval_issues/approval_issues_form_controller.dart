...
class ApprovalIssuesFormController extends HeaderController {
  @override
  ...
  final approvalissuesFormKey = GlobalKey<FormState>();
  ...
  Future submitApprovalIssuesForm(BuildContext context) async {
    if (approvalissuesFormKey.currentState!.validate()) {
      if (!isFormSubmit.value) {
        ...
        if (itemtagParticularList.value.isNotEmpty) {
          ...
          final CreateApprovalIssuesPaylaod payload =
          ...
          await ApprovalIssuesServices.createApprovalIssues(
  ...
  resetForm() {
    isFormSubmit.value = false;
    approvalissuesFormKey.currentState!.reset();
