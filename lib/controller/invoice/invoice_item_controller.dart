...
class InvoiceItemController extends HeaderController {
  final GlobalKey<ScaffoldState> scaffoldKey = GlobalKey<ScaffoldState>();

  final InvoiceController _invoiceController = Get.put(InvoiceController());

  final invoiceItemFormKey = GlobalKey<FormState>();
  ...
  Future getTagDetails(BuildContext context, String tagNumber) async {
    ...
    var resData = await InvoiceService.getTagItemdetails(
        context: context,
        tagnumber: tagNumber,
        metal: _invoiceController.selectedMetal.value?.value);
    ...
    if (resData != null) {
      ...
      if (data.calculationType == fixedRateCalcType) {
        ...
        if (_invoiceController.selectedGstType.value?.value == interGstType) {
        ...
        } else if (_invoiceController.selectedGstType.value?.value ==
            intraGstType) {
  ...
  void addParticularItem(BuildContext context) {
    if (invoiceItemFormKey.currentState!.validate()) {
      if (itemFormMode.value == 'add') {
        String isValidTag = _invoiceController.particulars
        ...
        if (isValidTag == '-1') {
          ...
          final ParticularDetails item = ParticularDetails(
              ...
              subItem: subItemController.text,
          ...
          _invoiceController.particulars.insert(0, item);
        ...
        int? index = _invoiceController.particulars
        ...
        final ParticularDetails item = ParticularDetails(
            ...
            wastageWeightType: wastageWeightType.value,
            makingChargeType: makingChargeType.value);
        _invoiceController.particulars[index] = item;
        resetForm(context: context);
      }
  ...
  void deleteParticularItem(BuildContext context, ParticularDetails item) {
    _invoiceController.particulars.removeWhere((i) => i.sNo == item.sNo);
    calculationBilling();
  }
  ...
  void editParticularItem(BuildContext context, ParticularDetails item) {
    ...
    subItemController.text = item.subItem.toString();
  ...
  void calculationBilling() {
    double totalAmount = 0.0;
    double gstAmount = 0.0;

    for (var i in _invoiceController.particulars.value) {
    ...
    _invoiceController.gstAmount(double.parse(gstAmount.toStringAsFixed(2)));
  ...
  Future getOrderDetails(BuildContext context, String orderNumber) async {
    retrieveOrderNumberLoading(true);
    var resData = await InvoiceService.getOrderdetails(
        context: context, ordernumber: orderNumber);

    if (resData != null) {
      OrderTagDetailsListData data =
          OrderTagDetailsListData.fromJson(resData['data']);

          if(data.itemDetails!.isNotEmpty) {
        _invoiceController.particulars([]);
        _invoiceController.orderId([]);
        _invoiceController.selectedMetal.value = null ;

      _invoiceController.selectedGstType(DropdownModel(
          value: data.gstType.toString(), label: data.gstType.toString()));
      _invoiceController.customerMobileController.text =
          data.customerMobile.toString();
      _invoiceController.findCustomer(
          _invoiceController.customerMobileController.text, context);
          _invoiceController.orderId.add(data.orderDetailsId.toString());
          _invoiceController.selectedMetal(DropdownModel(value: data.metalId.toString(), label: data.metalName.toString()));

      var uuid = Uuid();
      String id = uuid.v4();
      for (var i in data.itemDetails!) {
        final ParticularDetails particularDetails = ParticularDetails(
            sNo: id,
            tag: i.tagNumber,
            subItem: i.subItemDetailsName,
            ...
            gstPercent:
                _invoiceController.selectedGstType.value?.label == interGstType
                    ? i.interGstPercent
                    : _invoiceController.selectedGstType.value?.label ==
                            intraGstType
                        ? i.intraGstPercent
                        : 0.0,
            ...
            payableAmount:
                _invoiceController.selectedGstType.value?.label == interGstType
                    ...
                    : _invoiceController.selectedGstType.value?.label ==
                        ...
                        ? i.intraSaleValue
                        : 0.0);
        _invoiceController.particulars.add(particularDetails);

        log("particular  ${particularDetails.intergstAmount}");
      }

      _invoiceController.orderController.text =
          data.orderTotalAmount.toString();
      calculationBilling();
      _invoiceController.calculateBilling();
   _invoiceController.orderNumberController.clear();
    } else {
      toastification.show(
