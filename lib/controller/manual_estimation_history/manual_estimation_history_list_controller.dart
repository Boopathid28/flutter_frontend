...
import 'package:ausales_application/view/widgets/screen_widgets/manual_estimation_history/manual_estimation_view_popup.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

class  ManualEstimationHistoryListController extends HeaderController {
  @override
  ...
  TextEditingController estimtaitonEntryDateFilterController = TextEditingController();
     Rx<ManualRetrieveEstimationDetails?> currentestimationdetails = Rx<ManualRetrieveEstimationDetails?>(null);

     
  RxList<ManualRetrieveParticularDetails> particularData = <ManualRetrieveParticularDetails>[].obs;
  ...
  RxList<ManualEstimationHistoryListData> tableData = <ManualEstimationHistoryListData>[].obs;
  ...
  Future getEstimationList(BuildContext context) async {
    ...
    FetchManualEstimationHistoryPayload payload = FetchManualEstimationHistoryPayload(
    ...
    final Map<String, dynamic>? estimtaionList =
        await MannualEstimationService.fetchMannualEstimationList(payload: payload);
  ...
  Future<ManualRetrieveEstimationDetails?> getEstimationDetails(BuildContext context, String id) async {
    final ManualRetrieveEstimationDetails? data = await MannualEstimationService.retrieveMannualEstimationDetail(context: context, estimationId: id);
   ...
   Future getestimationDetails(
      BuildContext context, ManualEstimationHistoryListData item) async {

            final ManualRetrieveEstimationDetails? data = await MannualEstimationService.retrieveMannualEstimationDetail(context: context, estimationId: item.id.toString());
