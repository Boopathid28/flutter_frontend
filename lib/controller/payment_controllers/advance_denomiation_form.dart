...
class AdvanceDenomiationForm extends HeaderController {
  ...
  final AdvancePaymentController _advancePaymentController =
      Get.put(AdvancePaymentController());
  ...
  void deleteParticulars(String id) {
    _advancePaymentController.paymentParticulars.removeWhere((item) => item.sNo == id);
  }
  ...
  void submitPaymentForm(BuildContext context) {
    if (advancePaymentFormKey.currentState!.validate()) {
      if (formMode.value == "create") {
        ...
        _advancePaymentController.paymentParticulars.add(data);
      }
      resetForm();
    }
  }
   ...
   void calculateAdvance() {
    ...
    for (var item in _advancePaymentController.paymentParticulars) {
      totalAmount += item.paidAmount!;
    }
    _advancePaymentController.advanceAmountController.text = totalAmount.toString();
  }
