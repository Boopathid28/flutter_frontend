...
import 'package:ausales_application/view/widgets/payment_widgets/chit_scheme_payment/chit_scheme_payement_particulars.dart';
import 'package:ausales_application/view/widgets/payment_widgets/chit_scheme_payment/chit_scheme_payment_tag_particulars.dart';
import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:get/get.dart';
import 'package:toastification/toastification.dart';

class ChitSchemePaymentController extends GetxController {
  final InvoiceMakePaymentController _invoiceMakePaymentController = Get.put(InvoiceMakePaymentController());
  ...
  RxList<int> selectedTagItemIdsList = <int>[].obs;

  Future getChitSchemeDetails(BuildContext context, List<ChitSchemeListData> selectedChitSchemeList,
      List<ChitPaymentDenominationParticulars> chitPaymentDetailsList, List<RetrieveParticularDetails> taggedItems, Function() onComplete, List<RetrieveParticularDetails> selectedTagChitPaymentParticulars) async {
                                    ...
                                    : SizedBox(
                                        child: Column(
                                          children: [
                                              ...
                                              children: [
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    Obx(() => Text(
                                                      "${_invoiceMakePaymentController.totalPaidAmount.value}",
                                                      style: TextPalette
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    Obx(() => Text(
                                                      "${_invoiceMakePaymentController.totalBonusAmount.value}",
                                                      style: TextPalette
                                                          .viewDetailsDataTextStyle,
                                                    )),
                                                  ],
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    Obx(() => Text(
                                                      "${_invoiceMakePaymentController.totalPaidWeight.value ?? ""}",
                                                      style: TextPalette
                                                          .viewDetailsDataTextStyle,
                                                    )),
                                                  ],
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    Obx(() => Text(
                                                      "${_invoiceMakePaymentController.balancePaidWeight.value ?? ""}",
                                                      style: TextPalette
                                                          .viewDetailsDataTextStyle,
                                                    )),
                                                  ],
                                            ...
                                            ChitSchemePaymentTagParticulars(taggedItems: taggedItems, onComplete: onComplete, selectedTagChitPaymentParticulars: selectedTagChitPaymentParticulars)
                                          ],
                                        ),
                                      ))),
                            SizedBox(
  ...
  void calulateSelectedSchemes(
    ...
    _invoiceMakePaymentController.totalBonusAmount(double.parse(totalBonus.toStringAsFixed(2)));
    _invoiceMakePaymentController.totalPaidWeight(double.parse(totalWeight.toStringAsFixed(3)));
    _invoiceMakePaymentController.totalPaidAmount(double.parse(totalPaid.toStringAsFixed(2)));
    _invoiceMakePaymentController.balancePaidWeight(double.parse(totalWeight.toStringAsFixed(3)));
    _invoiceMakePaymentController.balancePaidAmount(double.parse(totalPaid.toStringAsFixed(2)));
  }

  void selecteTagItems(BuildContext context, RetrieveParticularDetails tag, int index, List<RetrieveParticularDetails> dataList, List<RetrieveParticularDetails> selectedTagChitPaymentParticulars) {
    
      if (selectedTagItemIdsList.contains(tag.id)) {
        selectedTagItemIdsList.removeWhere((item) => item == tag.id);

        var jsonData = selectedTagChitPaymentParticulars[index].toJson();
        _invoiceMakePaymentController.balancePaidWeight(jsonData['wastage_weight_reduced'] + _invoiceMakePaymentController.balancePaidWeight.value);
        jsonData['wastage_weight_reduced'] = 0.0;
        selectedTagChitPaymentParticulars[index] = RetrieveParticularDetails.fromJson(jsonData);
        selectedTagChitPaymentParticulars.removeWhere((item) => item.id == tag.id);
      } else {
        if (_invoiceMakePaymentController.balancePaidWeight.value > 0) {
          selectedTagItemIdsList.add(tag.id!);
          selectedTagChitPaymentParticulars.add(tag);
          
          if (tag.grossWeight! > _invoiceMakePaymentController.balancePaidWeight.value) {
            var jsonData = selectedTagChitPaymentParticulars[index].toJson();
            jsonData['wastage_weight_reduced'] = _invoiceMakePaymentController.balancePaidWeight.value;
            selectedTagChitPaymentParticulars[index] = RetrieveParticularDetails.fromJson(jsonData);
            _invoiceMakePaymentController.balancePaidWeight(0);
            _invoiceMakePaymentController.negativeBalancePaidWeight(_invoiceMakePaymentController.balancePaidWeight.value - (tag.grossWeight??0.0));
          } else if (tag.grossWeight! < _invoiceMakePaymentController.balancePaidWeight.value) {
            var jsonData = selectedTagChitPaymentParticulars[index].toJson();
            jsonData['wastage_weight_reduced'] = tag.grossWeight!;
            _invoiceMakePaymentController.balancePaidWeight(_invoiceMakePaymentController.balancePaidWeight.value - tag.grossWeight!);
            selectedTagChitPaymentParticulars[index] = RetrieveParticularDetails.fromJson(jsonData);
            _invoiceMakePaymentController.negativeBalancePaidWeight(_invoiceMakePaymentController.balancePaidWeight.value - (tag.grossWeight??0.0));
          } else {
            var jsonData = selectedTagChitPaymentParticulars[index].toJson();
