...
class FloorListController extends HeaderController {
  @override
  ...
  RxList<FloorListData> tableData = <FloorListData>[].obs;
  ...
  Future getfloorList(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchFloorListPayload payload = FetchFloorListPayload(
    ...
    final Map<String, dynamic>? floorlist =
        await FloorService.fetchFloorList(payload: payload);

    if (floorlist != null) {
      tableData(floorlist['data']);
      totalpages(floorlist['total_pages']);
    } else {
      tableData([]);
      totalpages(1);
    }
