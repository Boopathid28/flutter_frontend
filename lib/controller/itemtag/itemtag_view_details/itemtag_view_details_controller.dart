...
class ItemtagViewDetailsController extends GetxController {
  ...
  Future getItemTagList(BuildContext context) async {
    ...
    var data = await ItemtagFormService.getTagEntryDetailsById(
        context: context, tagEntryId: args['tagEntryId'].toString());
    if (data != null) {
      itemTagList([]);

      data['tag_details'].forEach((item) {
        var dictValue = item;
