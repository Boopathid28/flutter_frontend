...
class ManualInvoiceController extends HeaderController {
  ...
  RxList<ManualParticularDetails> particulars = <ManualParticularDetails>[].obs;
  RxList<ManualInvoiceOldItemDetails> oldParticulars = <ManualInvoiceOldItemDetails>[].obs;
  RxList<ManualInvoiceAdvanceDetails> advanceParticulars =
      <ManualInvoiceAdvanceDetails>[].obs;

  RxList<ManualOldPurchaseParticularsDetails> oldPurchaseParticulars =
      <ManualOldPurchaseParticularsDetails>[].obs;

  RxList<ManualPaymentDenominationDetails> paymentParticulars =
      <ManualPaymentDenominationDetails>[].obs;
  ...
  RxList<ManualParticularDetails> selectedTagChitPaymentParticulars =
      <ManualParticularDetails>[].obs;
  ...
  Future getManualOldMetalDetails(context) async {
      ...
      if (billingDate.text.isNotEmpty &&
        ...
        if (oldMetalController.text.isNotEmpty) {
          ...
          if (isValid) {
            ...
            if (data != null) {
              ManualOldPurchaseParticularsDetails manualOldPurchaseParticularsDetails =
                  ManualOldPurchaseParticularsDetails(
                      id: data.id.toString(),
                      oldGoldBillNo: data.oldGoldBillNumber,
                      totalAmount: data.oldGoldAmount.toString());
              oldPurchaseParticulars.add(manualOldPurchaseParticularsDetails);

              oldMetalController.clear();
            }

            calculateManualOldMetals();
  ...
  Future getManualSuspenseDetails(context) async {
    if (isBranchuser.value) {
      if (billingDate.text.isNotEmpty &&
          selectedBranch.value != null &&
          selectedGstType.value != null &&
  ...
  Future getManualAdvanceDetails(context) async {
    if (isBranchuser.value) {
      if (billingDate.text.isNotEmpty &&
          selectedBranch.value != null &&
          selectedGstType.value != null &&
  ...
  void applyManualAdvanceDetails(
    ...
    if ((advanceRedeemAmountController.text.isNotEmpty &&
      ...
      ManualInvoiceAdvanceDetails invoiceAdvanceDetails = ManualInvoiceAdvanceDetails(
  ...
  void calculateManualAdvanceValues() {
    double totalValue = 0.0;
    for (var i in advanceParticulars) {
      totalValue += i.totalAmount!;
    }
    advanceAmount(totalValue);
    calculateManualBilling();
  }

  void calculateManualOldMetals() {
    double totalValue = 0.0;
    for (var i in oldPurchaseParticulars.value) {
  ...
  Future createManualBilling(BuildContext context, bool print) async {
    if (particulars.length > 0) {
      if (!isSaveBillingLoading.value) {
        ...
        if (paymentParticulars.isNotEmpty ||
          ...
          ManualInvoicePaylaod payload = ManualInvoicePaylaod(
          ...
          final String? data = await ManualInvoiceService.createManualInvoice(
              context: context, payload: payload);
  ...
  Future createManualSuspense(BuildContext context) async {
    if (isBranchuser.value) {
  ...
  void onManualTotalPayableAmountChanged() {
    ...
    for (var i = 0; i < particulars.value.length; i++) {
      ManualParticularDetails item = particulars.value[i];
      ...
      particulars[i] = ManualParticularDetails.fromJson(itemJson);
    }
