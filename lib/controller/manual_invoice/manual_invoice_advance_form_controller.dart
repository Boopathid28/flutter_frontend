...
class ManualInvoiceAdvanceFormController extends HeaderController {
  ...
  final manualinvoiceAdvanceFormKey = GlobalKey<FormState>();
  ...
  Future CreateManualAdvance(BuildContext context) async {
    if (manualinvoiceAdvanceFormKey.currentState!.validate()) {
      if (!isFormSubmit.value) {
        isFormSubmit(true);
        CreateAdvancePayload payload = CreateAdvancePayload(
            advanceWeightPurity: selectedPurity.value?.value,
            branch: _manualInvoiceController.selectedBranch.value?.value,
            customerDetails: _manualInvoiceController.customerDetails.value.id.toString(),
