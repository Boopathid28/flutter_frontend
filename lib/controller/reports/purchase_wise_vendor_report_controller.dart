...
class PurchaseWiseVendorReportController extends HeaderController {
  ...
  RxList<PurchasewiseVendorReportListData> tableData =
      <PurchasewiseVendorReportListData>[].obs;
  ...
  Future getpurchasevendorwisereport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchPurchasewiseVendorReportListDataPayload payload =
        FetchPurchasewiseVendorReportListDataPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
    ...
    final Map<String, dynamic>? purchasevendordesignerwisereportList =
        await ReportsServices
            .fetchPurchasewisevendorReport(payload: payload);

    if (purchasevendordesignerwisereportList != null) {

 tableData(purchasevendordesignerwisereportList['list']);
    totalpages(purchasevendordesignerwisereportList['data']['total_pages']);
      totalPieces(purchasevendordesignerwisereportList['data']['total_pieces'].toString());
      totalGrossWeight(purchasevendordesignerwisereportList['data']['total_gross_weight'].toString());
      totalNetWeight(purchasevendordesignerwisereportList['data']['total_net_weight'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
   ...
   Future exportData(BuildContext context) async {
    ...
    final payload = FetchPurchasewiseVendorReportListDataPayload(
         page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            designer: vendordesignerFilter,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportPurchaseVendorwiseListReport(payload, context);
