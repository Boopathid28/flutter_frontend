...
class OrderSubItemWiseReportListController extends HeaderController {
  ...
  RxList<OrderSubItemWiseReportListData> tableData =
      <OrderSubItemWiseReportListData>[].obs;
  ...
  Future getordersubitemwisereport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchOrderSubItemWiseReportListPayload payload =
        FetchOrderSubItemWiseReportListPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            item: itemFilter,
    ...
    final Map<String, dynamic>? ordersubitemwisereportList =
        await ReportsServices
            .fetchOrderSubItemWiseReport(payload: payload);

    if (ordersubitemwisereportList != null) {

 tableData(ordersubitemwisereportList['list']);
    totalpages(ordersubitemwisereportList['data']['total_pages']);
      totalPieces(ordersubitemwisereportList['data']['total_pieces'].toString());
      totalGrossWeight(ordersubitemwisereportList['data']['total_gross_weight'].toString());
      totalNetWeight(ordersubitemwisereportList['data']['total_net_weight'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
   ...
   String? branchFilter;
    ...
    final payload = FetchOrderSubItemWiseReportListPayload(
       page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            item: itemFilter,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportOrderSubitemReport(payload, context);
