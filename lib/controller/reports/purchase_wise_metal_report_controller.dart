...
class PurchaseWiseMetalReportController extends HeaderController {
  ...
  RxList<PurchasewiseMetalRepaortListData> tableData =
      <PurchasewiseMetalRepaortListData>[].obs;
  ...
  Future getpurchasemetalwisereport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchPurchasewiseMetalRepaortListDataPayload payload =
        FetchPurchasewiseMetalRepaortListDataPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            metal: metalFilter,
    ...
    final Map<String, dynamic>? purchasemetalwisereportList =
        await ReportsServices
            .fetchPurchasewisemetalReport(payload: payload);

    if (purchasemetalwisereportList != null) {

 tableData(purchasemetalwisereportList['list']);
    totalpages(purchasemetalwisereportList['data']['total_pages']);
      totalPieces(purchasemetalwisereportList['data']['total_pieces'].toString());
      totalGrossWeight(purchasemetalwisereportList['data']['total_gross_weight'].toString());
      totalNetWeight(purchasemetalwisereportList['data']['total_net_weight'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
   ...
   Future exportData(BuildContext context) async {
    ...
    final payload = FetchPurchasewiseMetalRepaortListDataPayload(
        page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            metal: metalFilter,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportPurchaseMetalwiseListReport(payload, context);
