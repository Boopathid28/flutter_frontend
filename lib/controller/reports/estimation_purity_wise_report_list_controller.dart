...
class EstimationPurityWiseReportListController extends HeaderController {
  ...
  RxList<EstimationPurityWiseReportListData> tableData =
      <EstimationPurityWiseReportListData>[].obs;
  ...
  Future getestimationpuritywisereport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchEstimationPurityWiseReportListPayload payload =
        FetchEstimationPurityWiseReportListPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            purity: purityFilter,
    ...
    final Map<String, dynamic>? orderpuritywisereportList =
        await ReportsServices.fetchEstimationPurityWiseReport(payload: payload);
    ...
    if (orderpuritywisereportList != null) {
      ...
      totalGrossWeight(
          orderpuritywisereportList['data']['total_gross_weight'].toString());
      totalNetWeight(
          orderpuritywisereportList['data']['total_net_weight'].toString());
      totalAmount(orderpuritywisereportList['data']['total_amount'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
   ...
   String? branchFilter;
    ...
    final payload = FetchEstimationPurityWiseReportListPayload(
        page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            purity: purityFilter,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportEstimationPuritywiseReport(payload, context);
