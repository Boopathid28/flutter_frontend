...
class EstimationItemWiseReportListController extends HeaderController {
  ...
  RxList<EstimationItemWiseReportListData> tableData =
      <EstimationItemWiseReportListData>[].obs;
  ...
  Future getestimationitemwisereport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchEstimationItemWiseReportListPayload payload =
        FetchEstimationItemWiseReportListPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            item: itemFilter,
    ...
    final Map<String, dynamic>? estimationitemwisereportList =
        await ReportsServices.fetchEstimationItemWiseReport(payload: payload);

    if (estimationitemwisereportList != null) {
      tableData(estimationitemwisereportList['list']);
      totalpages(estimationitemwisereportList['data']['total_pages']);
      totalPieces(
          estimationitemwisereportList['data']['total_pieces'].toString());
      totalGrossWeight(estimationitemwisereportList['data']
              ['total_gross_weight']
          .toString());
      totalNetWeight(
          estimationitemwisereportList['data']['total_net_weight'].toString());
      totalAmount(
          estimationitemwisereportList['data']['total_amount'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
   ...
   String? branchFilter;
    ...
    final payload = FetchEstimationItemWiseReportListPayload(
         page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            item: itemFilter,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportEstimationItemwiseReport(payload, context);
