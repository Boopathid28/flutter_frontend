...
class VendorWiseRepairReportListController extends HeaderController {
  ...
  RxList<VendorWiseRepairReportListData> tableData =
      <VendorWiseRepairReportListData>[].obs;
  ...
  Future getvendorwiserepairreport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchVendorWiseRepairReportListPayload payload =
        FetchVendorWiseRepairReportListPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
    ...
    final Map<String, dynamic>? vendorwiserepairreportList =
        await ReportsServices
            .fetchvendorWiseRepairReport(payload: payload);

    if (vendorwiserepairreportList != null) {

 tableData(vendorwiserepairreportList['list']);
    totalpages(vendorwiserepairreportList['data']['total_pages']);
      totalPieces(vendorwiserepairreportList['data']['total_pieces'].toString());
      totalGrossWeight(vendorwiserepairreportList['data']['total_gross_weight'].toString());
      totalNetWeight(vendorwiserepairreportList['data']['total_net_weight'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
  ...
  Future exportData(BuildContext context) async {
    ...
    final payload = FetchVendorWiseRepairReportListPayload(
        page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            designer: vendordesignerFilter,
    ...
    var response =
        await ExportReportService.exportrepairvendorReport(payload, context);
