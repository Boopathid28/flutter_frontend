...
import '../../model/reports/vendor_purity_wise_sales_report_models.dart';

class VendorPurityWiseSalesReportController  extends HeaderController {
  @override
  ...
  RxList<VendorPurityWiseSalesReport> tableData = <VendorPurityWiseSalesReport>[].obs;
  ...
  Future getVendorPurityWisesalesReport() async {
    isTableLoading(true);
    ...
    if (selectedVendor.value != null && selectedVendor.value?.value != '0') {
      ...
      final payload = VendorPurityWiseSalesReportPayload(
          branch: branchFilter,
          fromDate:
          ...
          vendor: vendorFilter);
      var response =
          await ReportsServices.fetchVendorPurityWiseSalesReport(payload);
      if (response != null) {
   ...
   String? branchFilter;
    ...
    if (selectedVendor.value != null && selectedVendor.value?.value != '0') {
      ...
      final payload = VendorPurityWiseSalesReportPayload(
          ...
          toDate: toDateController.text,
          vendor: vendorFilter);
    var response = await ExportReportService.exportVendorPurityWiseSalesReport(
        payload, context);
