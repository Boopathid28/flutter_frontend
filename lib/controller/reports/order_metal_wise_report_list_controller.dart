...
class OrderMetalWiseReportListController extends HeaderController {
  ...
  RxList<OrderMetalWiseReportListData> tableData =
      <OrderMetalWiseReportListData>[].obs;
  ...
  Future getordermetalwisereport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchOrderMetalWiseReportListPayload payload =
        FetchOrderMetalWiseReportListPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            metal: metalFilter,
    ...
    final Map<String, dynamic>? ordermetalwisereportList =
        await ReportsServices
            .fetchOrderMetalWiseReport(payload: payload);
 ...
 tableData(ordermetalwisereportList['list']);
    totalpages(ordermetalwisereportList['data']['total_pages']);
      ...
      totalGrossWeight(ordermetalwisereportList['data']['total_gross_weight'].toString());
      totalNetWeight(ordermetalwisereportList['data']['total_net_weight'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
   ...
   Future exportData(BuildContext context) async {
    ...
    final payload = FetchOrderMetalWiseReportListPayload(
        page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            metal: metalFilter,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportOrdermetalReport(payload, context);
