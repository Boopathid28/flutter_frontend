...
class ApprovalIssuedReportController extends HeaderController {
  ...
  RxList<ApprovalIssueReportListData> tableData =
      <ApprovalIssueReportListData>[].obs;
  ...
  Future getApprovalIssuedreport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchApprovalIssueReportListDataPayload payload =
        FetchApprovalIssueReportListDataPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
    ...
    final Map<String, dynamic>? ordermetalwisereportList =
        await ReportsServices
            .fetchApprovalIssuedReport(payload: payload);

    if (ordermetalwisereportList != null) {

 tableData(ordermetalwisereportList['list']);
    totalpages(ordermetalwisereportList['data']['total_pages']);
      totalPieces(ordermetalwisereportList['data']['total_pieces'].toString());
      totalWeight(ordermetalwisereportList['data']['total_weight'].toString());
      totalItem(ordermetalwisereportList['data']['total_items'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
   ...
   Future exportData(BuildContext context) async {
    ...
    final payload = FetchApprovalIssueReportListDataPayload(
        page: page.value,
            fromDate: fromDate,
            toDate: toDate,
                  search: searchController.text,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportApprovalIssuedListReport(payload, context);
   ...
   Future exportPdfData(BuildContext context) async {
    ...
    final payload = FetchApprovalIssueReportListDataPayload(
        page: page.value,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportApprovalIssuedListPdfReport(payload, context);
