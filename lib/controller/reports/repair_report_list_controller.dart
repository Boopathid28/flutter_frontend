...
class RepairReportListController extends HeaderController {
  ...
  RxString totalVendorCharges = "0".obs;
  RxString totalCustomerCharges = "0".obs;

  RxBool isTableLoading = true.obs;
  RxBool isExportLoading = false.obs;
  RxBool isExportPdfLoading = false.obs;

  RxList<RepairReportListData> tableData =
      <RepairReportListData>[].obs;
  ...
  Future getRepairreport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchRepairReportListDataPayload payload =
    ...
    final Map<String, dynamic>? repairreportList =
        await ReportsServices
            .fetchRepairReport(payload: payload);

    if (repairreportList != null) {

 tableData(repairreportList['list']);
    totalpages(repairreportList['data']['total_pages']);
      totalPieces(repairreportList['data']['total_pieces'].toString());
      totalGrossWeight(repairreportList['data']['total_gross_weight'].toString());
      totalVendorCharges(repairreportList['data']['total_vendor_charges'].toString());
      totalCustomerCharges(repairreportList['data']['total_customer_charges'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
   ...
   Future exportData(BuildContext context) async {
    ...
    final payload = FetchRepairReportListDataPayload(
        page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            customer: customerFilter,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportRepairListReport(payload, context);
...
Future exportPdfData(BuildContext context) async {
    ...
    final payload = FetchRepairReportListDataPayload(
        page: page.value,
            ...
            menuId: await HomeSharedPrefs.getCurrentMenu());
    var response =
        await ExportReportService.exportRepairListPdfReport(payload, context);
