...
class MetalWiseRepairReportListController extends HeaderController {
  ...
  RxList<MetalWiseRepairListData> tableData =
      <MetalWiseRepairListData>[].obs;
  ...
  Future getmetalwiserepairreport(BuildContext context) async {
    isTableLoading(true);
    ...
    FetchMetalWiseRepairListPayload payload =
        FetchMetalWiseRepairListPayload(
            page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            metal: metalFilter,
    ...
    final Map<String, dynamic>? metalwiserepairreportList =
        await ReportsServices
            .fetchMetalWiseRepairReport(payload: payload);

    if (metalwiserepairreportList != null) {

 tableData(metalwiserepairreportList['list']);
    totalpages(metalwiserepairreportList['data']['total_pages']);
      totalPieces(metalwiserepairreportList['data']['total_pieces'].toString());
      totalGrossWeight(metalwiserepairreportList['data']['total_gross_weight'].toString());
      totalNetWeight(metalwiserepairreportList['data']['total_net_weight'].toString());
    } else {
      tableData([]);
      totalpages(1);
      totalPieces("0");
  ...
  Future exportData(BuildContext context) async {
    ...
    final payload = FetchMetalWiseRepairListPayload(
     page: page.value,
            fromDate: fromDate,
            toDate: toDate,
            metal: metalFilter,
    ...
    var response =
        await ExportReportService.exportrepairmetalReport(payload, context);
