...
class DailyStockItemWiseReportController extends HeaderController {
  @override
  ...
  RxString totalIssueWeight = "0".obs;
  RxString totalIssuePieces = "0".obs;
  RxString totalDifferentWeight = "0".obs;
  ...
  RxList<DailyStockItemWiseReportList> tableData =
      <DailyStockItemWiseReportList>[].obs;
  ...
  Future getDailyStockItemWiseReport() async {
    isTableLoading(true);
    ...
    final payload = DailyStockItemWiseReportPayload(
      branch: branchFilter,
      fromDate:
    ...
    var response = await ReportsServices.fetchDailyStockItemWiseReport(payload);
    if (response != null) {
      ...
      totalIssueWeight(response.totalIssuedWeight.toString());
      totalIssuePieces(response.totalIssuedPieces.toString());
      totalDifferentWeight(response.totalDifferenceWeight.toString());
    }else{
      tableData([]);
      totalpages(1);
      totalOutPieces("0");
  ...
  Future exportData(BuildContext context) async {
    ...
    var response =
        await ExportReportService.exportDailyStockItemWiseReport(payload, context);
   ...
   Future exportPdfData(BuildContext context) async {
    ...
    var response =
        await ExportReportService.exportDailyStockItemWisePdfReport(payload, context);
