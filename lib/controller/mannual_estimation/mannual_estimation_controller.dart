...
import 'package:ausales_application/view/widgets/screen_widgets/mannual_estimation/mannual_estimation_advance/mannual_estimation_retrieve_data_form.dart';
...
class MannualEstimationController extends HeaderController {
  ...
  RxList<MannualParticularDetails> particulars =
      <MannualParticularDetails>[].obs;
  RxList<MannualExchangeDetails> oldParticulars =
      <MannualExchangeDetails>[].obs;
  RxList<MannualEstimationAdvanceDetails> advanceParticulars =
      <MannualEstimationAdvanceDetails>[].obs;

  RxList<MannualOldPurchaseParticularsDetails> oldPurchaseParticulars =
      <MannualOldPurchaseParticularsDetails>[].obs;
  ...
  RxList<MannualParticularDetails> selectedTagChitPaymentParticulars =
      <MannualParticularDetails>[].obs;
        ...
        if (advanceController.text.isNotEmpty) {
          ...
          if (isValid) {
            ...
            if (data != null) {
              ...
              await Get.dialog(
                  barrierDismissible: false,
                  MannualEstimationRetrieveDataForm(data: data));
            }
          } else {
            isAdvanceFetchLoading(false);
            toastification.show(
  ...
  void applyAdvanceDetails(
    ...
    if ((advanceRedeemAmountController.text.isNotEmpty &&
      ...
      MannualEstimationAdvanceDetails estimationAdvanceDetails =
  ...
  Future createEstimation(BuildContext context, bool print) async {
    if (particulars.length > 0) {
      if (!isSaveEstimationLoading.value) {
        ...
        MannualEstimationPayload payload = MannualEstimationPayload(
            menuId: menuId,
        ...
        final String? data =
            await MannualEstimationService.createMannualEstimtaion(
                context: context, payload: payload);
        ...
        if (data != null) {
          if (print) {
            ...
            if (companyData != null) {
              final ManualRetrieveEstimationDetails? mannualestimationDetails =
                  await _manualEstimationHistoryListController
                      .getEstimationDetails(context, data.toString());

              if (mannualestimationDetails != null) {
                List<ManualRetrieveParticularDetails> itemsList =
                    <ManualRetrieveParticularDetails>[
                  ...mannualestimationDetails.particularDetails!
                ];
                ...
                await Get.dialog(
                    ...
                    MannualEstimationPrintout(
                        ...
                        estimationDetails: mannualestimationDetails,
                        items: itemsList));
  ...
  void onTotalPayableAmountChanged() {
    ...
    for (var i = 0; i < particulars.value.length; i++) {
      MannualParticularDetails item = particulars.value[i];
      ...
      particulars[i] = MannualParticularDetails.fromJson(itemJson);
    }
    ...
    balanceAmount(userTotalPaybale - (exchange + advance + paid + chitScheme));
  }

  Future getPrint(BuildContext context) async {
    final CompanyDetailsData? companyData =
        await CompanyService.retrieveCompany(context: context);
    ...
    if (companyData != null) {
      ...
      int pieces = 0;
      double weight = 0.0;
      for (var i in particulars.value) {
        log(i.toJson().toString());
        itemsList.add(ManualRetrieveParticularDetails.fromJson(i.toJson()));
        pieces += i.pieces ?? 0;
        weight += i.netWeight ?? 0.0;
      }
      itemsList.add(ManualRetrieveParticularDetails(
          pieces: pieces,
          netWeight: weight,
          payableAmount: payableAmount.value));

      ManualRetrieveEstimationDetails mannualestimationDetails =
        ...
        gstAmount: gstAmount.value,
        advanceAmount: advanceAmount.value,
        customerDetailsAddress: customerDetails.value.address,
        discountAmount: double.parse(
            discountController.text.isEmpty ? "0" : discountController.text),
        roundoffAmount: double.parse(
            roundOffController.text.isEmpty ? "0" : roundOffController.text),
        exchangeAmount: exchangeAmount.value,
        chitAmount: chitAmount.value,
        saleReturnAmount: saleReturnAmount.value,
        ...
        customerDetailsName: customerDetails.value.customerName,
        customerDetailsMobile: customerDetails.value.mobileNumber,
      );
    }
  }
}
