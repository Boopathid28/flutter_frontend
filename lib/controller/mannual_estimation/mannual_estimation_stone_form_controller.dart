...
class MannualEstimationStoneFormController extends GetxController {
  ...
  Future submitEstimationStoneForm(BuildContext context) async {
    if (estimationStoneDetailsFormKey.currentState!.validate()) {
      ...
      if (stoneFormMode.value == 'create') {
        var uuid = Uuid();
        final MannualTagDetailStoneDetails stoneDetails = MannualTagDetailStoneDetails(
          id: uuid.v4(),
        ...
        _mannualEstimationItemController.stoneParticularList.insert(0,stoneDetails);
        resetStoneForm(context);
      } else {
        int index = _mannualEstimationItemController.stoneParticularList.indexWhere((item) => item.id == stoneFormUpdateId.value);
        final MannualTagDetailStoneDetails stoneDetails = MannualTagDetailStoneDetails(
        ...
        _mannualEstimationItemController.stoneParticularList[index] = stoneDetails;
        resetStoneForm(context);
      }
    }
  }
  
  void editItemtagStone(BuildContext context, MannualTagDetailStoneDetails item) {
  ...
  void deleteItemtagStone(BuildContext context, String index) {
    _mannualEstimationItemController.stoneParticularList.removeWhere((item) => item.id == index);
    Get.back();
  }
  ...
  void stoneAddCompleted(BuildContext context) {
    ...
    log("stoneamt ${amount}");
    for (var i in _mannualEstimationItemController.stoneParticularList) {
      if (i.reduceWeight!) {
    ...
    _mannualEstimationItemController.totalStonePieces(pieces);
    _mannualEstimationItemController.totalStoneWeight(amount);
    _mannualEstimationItemController.stoneAmountController.text = amount.toStringAsFixed(2);

    _mannualEstimationItemController.reduceStoneWeight(reduceWeight);
