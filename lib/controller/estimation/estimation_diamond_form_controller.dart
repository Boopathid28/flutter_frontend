...
class EstimationDiamondFormController extends GetxController {
  ...
  Future submitItemtagStoneForm(BuildContext context) async {
    if (estimationDiamondDetailsFormKey.currentState!.validate()) {
      ...
      if (diamondFormMode.value == 'create') {
        ...
        _estimationItemController.diamondParticularList.insert(0, diamondDetails);
        resetDiamondForm(context);
      } else {
        int index = _estimationItemController.diamondParticularList.indexWhere((item) => item.id == diamondFormUpdateId.value);
        final TagDetailDiamondDetails diamondDetails = TagDetailDiamondDetails(
        ...
        _estimationItemController.diamondParticularList[index] = diamondDetails;
        resetDiamondForm(context);
      }
    }
  }
  ...
  void deleteItemtagDiamond(BuildContext context, String index) {
    _estimationItemController.diamondParticularList.removeWhere((item) => item.id == index);
    Get.back();
  }
  ...
  void diamondAddCompleted(BuildContext context) {
    ...
    for (var i in _estimationItemController.diamondParticularList) {
    ...
    _estimationItemController.totalDiamondPieces(pieces);
    _estimationItemController.totalDiamondWeight(amount);
    _estimationItemController.diamondAmountController.text = amount.toStringAsFixed(2);

    _estimationItemController.reduceDiamondWeight(reduceWeight);
